...
export async function newGame() {
  ...
  if (state.gameMode === 'daily' && state.dailySeed) {
    // Phase 1: resolve the canonical board for this date. We try Firebase
    // first — if the date has been pre-generated by GitHub Actions or
    // already played by another client today, that's the authoritative
    // board and we use it verbatim. Otherwise we generate locally and
    // write our result up so the next visitor sees the same thing.
    //
    // Practice-daily (`?seed=` URL param) and offline play skip the
    // canonical fetch: practice is per-user and shouldn't pollute the
    // shared canonical bucket; offline gets a local-only board.
    const wantCanonical = !state.isDailyPractice;
    let reconstructed = null;
    if (wantCanonical) {
      try {
        const canonicalRaw = await loadDailyBoard(state.dailySeed);
    ...
    if (reconstructed) {
      ...
      if (wantCanonical) {
        saveDailyBoard(state.dailySeed, serializeBoard({
          ...
          activeGimmicks: state.activeGimmicks, codeVersion: 'v1.5.19',
        })).catch(() => {});
      }
    }
