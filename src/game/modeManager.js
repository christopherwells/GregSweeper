...
import { $, $$ } from '../ui/domHelpers.js?v=1.0.9';
import { newGame } from './gameActions.js?v=1.0.9';
import { persistGameState, tryResumeGame } from './gamePersistence.js?v=1.0.9';
import { loadCheckpoint, loadStats } from '../storage/statsStorage.js?v=1.0.9';
import { CHAOS_UNLOCK_LEVEL } from '../logic/difficulty.js?v=1.0.9';
...
export function updateModeUI(mode) {
  ...
  if (mode === 'skillTrainer') {
    ...
    import('../ui/skillTrainerUI.js?v=1.0.9').then(m => m.showSkillTrainer()).catch(err => {
    ...
    import('../ui/skillTrainerUI.js?v=1.0.9').then(m => m.hideSkillTrainer()).catch(() => {});
  }
}
