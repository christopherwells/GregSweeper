...
export function serializeBoard({ board, rows, cols, totalMines, rngSeed, activeGimmicks, codeVersion, firstClick }) {
  ...
  // tiling days; a rectangular board emits none of these
  ...
  // certify from the board's center, and on a rectangle that is the container
  // center, which is why it has never needed storing. On a tiling the
  // container is an arbitrary exact factorization (63 hexagons ship as 7×9), so
  // the container center is an unrelated cell and re-certifying from it proves
 ...
 * board grid and side metadata so the caller can splice into
...
export function deserializeBoard(raw) {
  ...
  // consumer. A stored `firstClick` wins; otherwise it is the container center,
  ...
  // because the container-center formula was hand-copied into the nightly
