...
- Every 3rd daily (day-of-month divisible by 3) is an "improvement day": the client generates 10 candidate seeds (`${dateString}:trial0`..`trial9`), solves each, and picks the one whose board maximises the currently-targeted feature. Other days are plain date-seeded dailies, unbiased.
- The target feature is chosen server-side by the R refit: after each fit, the coefficient with the highest posterior coefficient of variation (SD / |mean|) from a whitelist of push-able features wins, and its name is written to `src/logic/experimentTarget.json`. The whitelist excludes features that either can't be meaningfully pushed (`passAMoves` always non-zero) or that we don't want to inflate (`cellCount`). The client fetches this JSON at startup via `loadExperimentTarget()` in `src/logic/experimentDesign.js`.
