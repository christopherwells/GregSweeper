...
Message text rotates day-to-day from a small per-category pool (`DAILY_BODIES`, `WEEKLY_BODIES` in `send-push.mjs`). Pick is deterministic by date so all subscribers on the same day see the same line, but the wording varies day-to-day instead of repeating "Today's daily is waiting" forever. Categories: `weekly` (Monday — overrides daily), `daily` (default).
...
- Categories: `weekly` (Mon morning), `daily` (default). The cron picks based on the calendar; per-category opt-out via `notificationPrefs.dailyReminder` (default true).
