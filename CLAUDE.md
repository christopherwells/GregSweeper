...
- Guards: `MIN_SCORES_TO_FIT = 30` (priors do the regularisation, so the old `MAX_COEF_DRIFT` clamp was retired), `MIN_PLAYS_FOR_HANDICAP = 3` (only affects the residual-fallback path), `ADAPT_DELTA = 0.99` with `MAX_DIVERGENT_FRAC = 0.25%` — a fit that comes back with Rhat > 1.05, ESS < 400, or more than 0.25% divergent transitions is rejected and the previous `PAR_MODEL` stays.
